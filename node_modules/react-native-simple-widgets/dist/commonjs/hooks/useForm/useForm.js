...
const defaultFunc = () => true; // If the initial value is not empty, it should be touched to show the green check mark
...
const useForm = (initialValues = {}, onSubmit, validationSchema = {}, options = {}) => {
  const {
  ...
  React.useEffect(() => {
    setHasMissingField(Object.keys(errors).some(key => isRequiredError(errors[key])));
  }, [errors]);

  const handleSubmit = () => {
    touchAllFields();
    const errors = validateAllFields();
    setErrors(errors); // If there is no errors, submit value else abort
  ...
  const handleBlur = name => () => {
    runFieldLevelValidations(ValidateOnEvent.ValidateOnBlur)(name, values[name]);
    setTouched(previousTouched => ({ ...previousTouched,
      [name]: true
    }));
  ...
  const validatorOnEvent = (name, rule, validatorEvent) => {
    const {
      [validatorEvent]: validatorFlag = true
    } = rule;
    return validatorFlag || touched[name] && errors[name] && errors[name].error && !isRequiredError(errors[name]);
  };
  ...
  const validateByFieldName = (name, value = "", validatorEvent) => {
    ...
    const skipValidations = (fieldValidatorSchema === null || fieldValidatorSchema === void 0 ? void 0 : fieldValidatorSchema.optional) && value.trim() === "";
    ...
    fieldValidatorSchema && Object.keys(fieldValidatorSchema).some(key => {
      const validator = _Validator.default[key]; // ignore invalid validation props
  ...
  const resetForm = React.useCallback((initial = {}) => {
    setValues(initial);
    setErrors({});
    setTouched(getInitialTouched(initial)); // eslint-disable-next-line
  }, [setValues]);
  return {
    values,
    errors,
