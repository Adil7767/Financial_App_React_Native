import { format, parse, parseISO, differenceInCalendarDays, differenceInYears, differenceInDays, compareAsc, isValid, startOfDay } from "date-fns";
...
const countNights = (checkIn, checkOut) => differenceInCalendarDays(new Date(checkOut), new Date(checkIn));

const parseAPIDay = (day, dateFormat = API_DATE_FORMAT) => parse(day, dateFormat, new Date());

const parseFormDay = (day, dateFormat = DATE_FORMAT_DEFAULT) => parse(day, dateFormat, new Date());

const isValidTime = (time, dateFormat = DATE_FORMAT_DEFAULT) => isValid(parseFormDay(time, dateFormat));

const getDateFormat = () => DATE_FORMAT_DEFAULT;

const getDurationInDays = (startTime, endTime) => differenceInDays(parseAPIDay(endTime), parseAPIDay(startTime));

const countYears = (startDate, endDate) => differenceInYears(new Date(endDate), new Date(startDate));
...
const differenceInDaysFromCurrentDate = checkin => differenceInDays(parseAPIDay(checkin), startOfDay(new Date()));

export { DAY_WITHOUT_YEAR_FORMAT, API_DATE_FORMAT, SHORT_DATE_FORMAT, REVERSE_SHORT_DATE_FORMAT, API_DURATION_FORMAT, API_DATETIME_FORMAT, DURATION_FORMAT, DATE_FORMAT_DEFAULT, ISO_DATE_FORMAT, TIMELINE_LOCAL_TIME_LONG_FORMAT, TIMELINE_LOCAL_TIME_SHORT_FORMAT, TIMELINE_DESTINATION_TIME_FORMAT, TIME_FORMAT, getDateFormat, formatTime, formatZonedTime, formatAPIDayParam, formatDisplayDay, formatRefundDate, formatDisplayDuration, formatShortDate, formatShortLocalTime, formatLongLocalTime, formatLongerLocalTime, formatDestinationDate, parseAPIDay, countNights, parseFormDay, compareTime, isValidTime, getDurationInDays, countYears, convertTimeFormat, convertToDestinationTimezone, differenceInDaysFromCurrentDate };
