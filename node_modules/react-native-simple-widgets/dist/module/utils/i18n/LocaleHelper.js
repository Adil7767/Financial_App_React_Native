...
export const numberFormat = (number = 0, formatOptions = {}, locale = getCurrentLocale() || defaultLocale) => new Intl.NumberFormat(locale, {
  style: "decimal",
  ...formatOptions
}).format(number);
export const listFormat = (list = [], formatOptions = {}, locale = getCurrentLocale() || defaultLocale) => // @ts-ignore
...
export const getRoomBedTypeAndRoomSize = ({
  bedSize,
  squareMeterArea
}) => "".concat(bedSize ? bedSize : "").concat(bedSize && squareMeterArea ? " | " : "").concat(squareMeterArea ? translate("roomCard.squareMeter", {
  squareMeterArea
}) : "");
export const createScopedTranslate = scopedKey => (key, ...restArgs) => translate("".concat(scopedKey, ".").concat(key), ...restArgs);
